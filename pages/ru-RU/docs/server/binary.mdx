...
Пример: система: Linux amd64, пользователь: root, текущая директория: /root
...
6. Если файл конфигурации пуст или не существует, служба будет запущена с конфигурацией по умолчанию, путь к файлу конфигурации: `./etc/ppanel.yaml`,
...
Если вы используете службу прокси Cloudflare, вам нужно получить реальный IP-адрес пользователя. Пожалуйста, добавьте в раздел Http вашего конфигурационного файла Nginx:

- Необходимый модуль: **ngx_http_realip_module**, проверьте, скомпилирован ли этот модуль в nginx с помощью команды nginx -V, если нет, вам нужно будет скомпилировать его самостоятельно.
