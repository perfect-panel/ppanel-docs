...
O PPanel suporta inicialização baseada na Web, simplificando o processo de configuração manual.

### Passos de Inicialização

1. **Crie o arquivo de configuração necessário**: Primeiro, crie e configure manualmente o arquivo `/app/etc/ppanel.yaml` para as configurações subsequentes.
...
Aqui está um exemplo de opções de configuração para o `ppanel.yaml`:
