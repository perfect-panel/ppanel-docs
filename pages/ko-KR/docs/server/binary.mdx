...
6. 구성 파일이 비어 있거나 존재하지 않는 경우, 서비스는 기본 구성으로 시작되며, 구성 파일 경로는 `./etc/ppanel.yaml`입니다. 시스템 구성을 초기화하려면 `http://서버주소:8080/init`를 통해 진행하십시오.

#### NGINX 역방향 프록시 구성

다음은 `ppanel` 서비스를 `api.ppanel.dev` 도메인으로 프록시하는 역방향 프록시 구성 예시입니다.
...
Cloudflare 프록시 서비스를 사용하는 경우, 사용자 실제 접근 IP를 가져와야 합니다. Nginx 구성 파일의 Http 섹션에 다음을 추가하십시오:

- 필요 모듈: **ngx_http_realip_module** 모듈, `nginx -V` 명령어로 Nginx가 해당 모듈로 컴파일되었는지 확인하고, 없다면 직접 컴파일해야 합니다.
