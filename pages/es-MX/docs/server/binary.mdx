...
2. Servicio systemd:
   ...
   - Autostart del servicio: systemctl enable ppanel

3. Para habilitar el autostart al iniciar el sistema, puedes usar el siguiente comando:
...
6. Cuando el archivo de configuración está vacío o no existe, el servicio se iniciará con la configuración predeterminada, cuya ruta es: `./etc/ppanel.yaml`.
...
A continuación se muestra un ejemplo de configuración de proxy inverso, que proxy el servicio `ppanel` al dominio `api.ppanel.dev`.
