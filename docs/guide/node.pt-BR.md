...
# Editar o config, consulte as opções de configuração e salve antes de continuar
...
ConnetionConfig:
  Handshake: 4 # Limite de tempo de handshake, em segundos
...
Nodes:
  - PanelType: 'PPanel' # Tipo de painel: PPanel, SSpanel, V2board, NewV2board, PMpanel, Proxypanel, V2RaySocks
    ApiConfig:
      ...
      SpeedLimit: 0 # Mbps, configurações locais substituirão configurações remotas, 0 significa desativar
      DeviceLimit: 0 # Configurações locais substituirão configurações remotas, 0 significa desativar
    ...
    ControllerConfig:
      ...
      FallBackConfigs: # Suporta múltiplos fallback
        - SNI: # TLS SNI (Server Name Indication), vazio para qualquer
          ...
          ProxyProtocolVer: 0 # Enviar versão do protocolo PROXY, 0 para desativar
      CertConfig:
        CertMode: dns # Opção sobre como obter o certificado: none, file, http, dns. Escolher "none" desativará forçadamente a configuração tls.
